...
def getPredictionSuffix(route, destinationName, destinationId, isScheduled):
    if isScheduled:
        ...
        if destinationId == "30203" and destinationName == "Linden": # Linden = nothing
            return ""
        elif destinationId == "30203" and destinationName == "Loop": # TODO northbound express
            return "L"
...
def getPredictions(predConfig):
    preds = []
    # key = 
    resp = http.get(getApiUrl(predConfig, CTA_API_KEY))
    if resp.status_code != 200:
        fail("CTA request failed with: %d, %s", resp.status_code, resp.body())

    if resp.body()[0] == "<":
        return preds
    ctattJson = resp.json()["ctatt"]
    if "eta" not in ctattJson:
        return preds

    for pred in ctattJson["eta"]:
...
def renderPredictions(r, predConfig):
...
def main(config):
    ...
    return render.Root(
        render.Column(
            children = [
                render.Box(width = 1, height = 1),
                renderPredictions(render, firstLineConfig),
                renderAnimatedTrain(render, showAnimatedTrain),
                renderPredictions(render, secondLineConfig)
            ]
        )
    )
    ...
    return schema.Schema(
        ...
        fields = [
            schema.Text(
                id = "firstStop",
                name = "First Stop",
                desc = "The ID of stop in the top slot. Find Stop ID at https://www.transitchicago.com/developers/ttdocs",
                icon = "location-dot",
            ),
            schema.Dropdown(
                id = "firstRoute",
                name = "First Route",
                desc = "The route in the top slot",
                icon = "route",
                default = routeOptions[1].value,
            ...
            schema.Text(
                id = "secondStop",
                name = "Second Stop",
                desc = "The ID of stop in the bottom slot. Find Stop ID at https://www.transitchicago.com/developers/ttdocs",
                icon = "location-dot",
            ),
            schema.Dropdown(
                id = "secondRoute",
